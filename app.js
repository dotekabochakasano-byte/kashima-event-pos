...
function renderAdmin(){const list=el('adminList');if(!list)return;list.innerHTML='';products.forEach(p=>{const row=document.createElement('div');row.className='admin-row'+(p.active?'':' off');row.innerHTML=`<div class="admin-main"><strong>${escapeHtml(p.name)}</strong><small>${fmt(p.price)} / ${escapeHtml(p.category||'')}</small></div><button class="mini" title="上へ">↑</button><button class="mini" title="下へ">↓</button><button class="mini edit">編集</button><button class="mini toggle">${p.active?'停止':'再開'}</button>`;const [up,down]=row.querySelectorAll('.mini');up.onclick=()=>moveProduct(p.id,-1);down.onclick=()=>moveProduct(p.id,1);row.querySelector('.edit').onclick=()=>openEdit(p.id);row.querySelector('.toggle').onclick=()=>toggleProduct(p.id);list.appendChild(row)})}
async function moveProduct(id,dir){const idx=products.findIndex(p=>p.id===id);const target=idx+dir;if(target<0||target>=products.length)return;const a=products[idx],b=products[target];const ao=a.order,bo=b.order;a.order=bo;b.order=ao;await put('products',a);await put('products',b);await loadProducts()}
...
async function init(){
  ...
  el('openAdmin').onclick=()=>el('adminDialog').showModal();el('addProduct').onclick=addProduct;el('saveEdit').onclick=saveEdit;const checkoutBtn=el('checkout');checkoutBtn.onclick=handleCheckoutActivation;checkoutBtn.addEventListener('pointerup',handleCheckoutActivation,{passive:false});checkoutBtn.addEventListener('touchend',handleCheckoutActivation,{passive:false});document.addEventListener('pointerup',e=>{const b=e.target&&e.target.closest?e.target.closest('#checkout'):null;if(b)handleCheckoutActivation(e)},{capture:true,passive:false});el('clearCart').onclick=()=>{cart.clear();renderCart()};el('tendered').onclick=openTenderDialog;el('tendered').onfocus=e=>e.target.blur();
  ...
  if('serviceWorker'in navigator){try{await navigator.serviceWorker.register('./sw.js?v=11')}catch(e){console.warn('SW register failed',e)}}
}
